...
CREATE TABLE IF NOT EXISTS public.leave_requests_additional (
...
ADD CONSTRAINT fk_leave_requests_additional_user_id 
...
CREATE TABLE IF NOT EXISTS public.admin_granted_leaves (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id TEXT NOT NULL,
  leave_type_id UUID NOT NULL,
...
ADD CONSTRAINT fk_admin_granted_leaves_user_id 
FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
...
ALTER TABLE public.admin_granted_leaves 
ADD CONSTRAINT fk_admin_granted_leaves_granted_by 
