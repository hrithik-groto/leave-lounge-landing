...
-- Fix the trigger function to properly handle NULL values and half-day calculations
...
BEGIN
    ...
    IF NEW IS NOT NULL THEN
        IF COALESCE(NEW.is_half_day, false) = true THEN
            v_days_used := 0.5;
        ELSE
            v_days_used := GREATEST(1, EXTRACT(DAY FROM (NEW.end_date - NEW.start_date)) + 1);
        END IF;
...
-- Also fix the get_or_create_monthly_balance function to handle NULLs better
...
BEGIN
    ...
    IF v_balance IS NULL THEN
        ...
        BEGIN
            ...
            -- Insert new monthly balance with proper NULL handling
            INSERT INTO user_monthly_leave_balances (
                user_id, leave_type_id, year, month, 
                allocated_balance, used_balance, carried_forward
            ) VALUES (
